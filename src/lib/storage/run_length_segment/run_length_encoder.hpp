  ...
  std::shared_ptr<BaseEncodedSegment> _on_encode(const AnySegmentIterable<T> segment_iterable,
    ...
    segment_iterable.with_iterators([&](auto it, auto end) {
      ...
      for (; it != end; ++it) {
        ...
        const auto position_is_null = segment_value.is_null();
        is_current_null = position_is_null;
        segment_contains_null_values |= position_is_null;
    ...
    std::optional<pmr_vector<bool>> null_values_opt;
    if (segment_contains_null_values) {
      null_values_opt = std::move(null_values);
    }

    return std::make_shared<RunLengthSegment<T>>(values, null_values_opt, end_positions);
  }
};
